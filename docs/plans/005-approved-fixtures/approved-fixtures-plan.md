...
|--------|-----|------|--------|---------|-----------|-------|
| [ ] | T001 | Verify algorithm determinism — run SplitCalculation, DebtSimplification on test inputs 10x each, confirm identical output every time | financial-core | (exploratory — no file changes) | All 10 runs produce identical outputs for each scenario | Per finding 01; if non-deterministic, STOP and report |
| [ ] | T002 | Create `ApprovedFixtures/` directory and add JSON fixture files for rounding scenarios | financial-core | `CostSharingApp/tests/CostSharingApp.Tests/ApprovedFixtures/rounding-scenarios.json` | JSON file contains exact expected outputs for: $100/3, $10/3, $1/3, $99.99/7, $100.01 50/50, decimal percentages 33.33/33.33/33.34 of $100 | Per finding 04 — first participant gets remainder |
| [ ] | T003 | Create JSON fixture file for simplification scenarios | financial-core | `CostSharingApp/tests/CostSharingApp.Tests/ApprovedFixtures/simplification-scenarios.json` | JSON contains exact transaction routing for: circular 3-person, complex 4-person, symmetric 2-creditor, large amounts | Per finding 01 — verified deterministic in T001 |
| [ ] | T004 | Create JSON fixture file for end-to-end pipeline scenario | financial-core | `CostSharingApp/tests/CostSharingApp.Tests/ApprovedFixtures/pipeline-scenarios.json` | JSON contains full flow: expense inputs → split outputs → debt outputs → simplified debts → post-settlement remainder | Scenario 5 from research dossier |
| [ ] | T005 | Create ApprovedFixtureTests.cs test class | financial-core | `CostSharingApp/tests/CostSharingApp.Tests/ApprovedFixtures/ApprovedFixtureTests.cs` | Test class loads each JSON fixture, runs services with fixture inputs, asserts outputs match exactly | Uses fixed GUIDs per finding 02; uses System.Text.Json for deserialization |
| [ ] | T006 | Update .csproj to include JSON fixtures in test output | financial-core | `CostSharingApp/tests/CostSharingApp.Tests/CostSharingApp.Tests.csproj` | `<Content Include="ApprovedFixtures/**/*.json" CopyToOutputDirectory="PreserveNewest" />` added | Per finding 03 |
| [ ] | T007 | Run `just validate` — all tests pass (91 existing + new fixture tests) | financial-core | — | Exit code 0, all tests green | Final verification |

### Acceptance Criteria

- [ ] AC1: `CostSharingApp/tests/CostSharingApp.Tests/ApprovedFixtures/` directory exists with JSON golden files
- [ ] AC2: Rounding tests assert exact per-person amounts (who gets the extra cent)
- [ ] AC3: Simplification tests assert exact transaction routing (from → to → amount)
- [ ] AC4: At least one end-to-end fixture covers expense → split → debt → simplification → post-settlement
- [ ] AC5: All fixture tests pass with current service implementations (no logic changes)
- [ ] AC6: JSON files are human-readable and produce clear diffs
- [ ] AC7: Rounding gauntlet ($1/3, $10/3, $100/3, $99.99/7) has committed exact outputs
